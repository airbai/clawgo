 ...
 *   node scripts/claw_go_runtime_demo.js proactive pro Asia/Shanghai
 *   node scripts/claw_go_runtime_demo.js command free "/clawgo plan"
 */
...
function buildPayload(userState, eventType) {
  ...
  return {
    ...
    image_prompt: `cute red crayfish traveler selfie in ${destination.name}, ${eventType}, lively, cinematic`,
    voice_script: `主人，我在${destination.name}又解锁了新奇遇。想让我继续追这条线吗？`,
...
function handleCommand(input, userState) {
  if (input.startsWith("/clawgo status")) {
    return {
      ok: true,
      type: "status",
      data: {
  ...
  if (input.startsWith("/clawgo plan")) {
    return {
      ...
      data: {
        ...
        pro_limit: PRO_LIMIT,
        pro_features: ["HD image", "rare arc", "premium voice", "3 pushes/day"]
      }
    };
  }

  if (input.startsWith("/clawgo send")) {
  ...
  return {
    ...
    message: "支持: /clawgo status | /clawgo plan | /clawgo send"
  };
}
